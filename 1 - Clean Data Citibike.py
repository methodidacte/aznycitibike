...
# Save dataframe as table on the cluster
df.write.option("mode","overwrite").format("parquet").saveAsTable("CitibikeNY20")

# COMMAND ----------

# MAGIC %sql
# MAGIC 
# MAGIC SELECT COUNT(1) AS Nb_rows, MIN(start_time), MAX(stop_time) FROM CitibikeNY20

# COMMAND ----------

# export csv

datasetPath = "dbfs:/mnt/nycitibike/GOLD/golden_dataset"
df.coalesce(1).write.mode("overwrite").format("csv").option("overwriteSchema", "true").save(datasetPath)

file = dbutils.fs.ls(datasetPath)[-1].path
dbutils.fs.cp(file, datasetPath + ".csv")
dbutils.fs.rm(datasetPath, recurse=True) 